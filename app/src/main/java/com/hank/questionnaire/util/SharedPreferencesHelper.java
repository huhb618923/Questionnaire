...
/**
 * Created by facebank on 16-1-6.
 */
public class SharedPreferencesHelper {

    public static final String PREFERENCE_NAME = "ouding_preferences";

    public static SharedPreferences getSharePreferences() {
        return QuestionnairesApplication.getApplication().getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE);
    }

    public static SharedPreferences getUserIdPreferences(String packName) {
        if (!TextUtils.isEmpty(packName)) {
            try {
                Context context = QuestionnairesApplication.getApplication().createPackageContext(packName, Context.CONTEXT_IGNORE_SECURITY);
                if (context != null) {
